...
- 验证与收敛：是否由 native runtime 直接完成日常验证，是否只在用户显式要求时使用 `$to-review` / `$to-consolidation`，以及 durable document 是否遵守 owning Skill 的语义合同。
...
- 创建或修改 Sky Flow durable document 时按对应 owning Skill 做 Self-Review；单纯写复盘报告不增加额外文档流程。

## Boundaries

- 不做普通代码风险 review；需要专项实现风险审查时建议 `$to-review`。
- 不收敛 pending diff；需要专项处理补丁式实现和 fan-in 残留时建议 `$to-consolidation`。
- 不替代 owning Skill 的 durable-document 语义检查。
