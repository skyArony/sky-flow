...
7. 创建或更新 artifact：
   - plan-scoped：`tasks/<plan-id>/<task-id>.md`，维护 plan frontmatter 的 `tasks` 列表，并在 plan 正文保留 task topology 摘要。
...
- 完成后由 `to-implement` 精简自身正文并移入 `tasks/standalone/done/`；不经过 plan `to-archive` 压缩。
