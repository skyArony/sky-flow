...
自动收敛默认保持行为不变。允许将不触及外部契约、兼容、安全、持久化、状态机、并发、可观测性或用户可见结果的微小且合理行为差异纳入简化；必须在结果中明确说明差异与理由。边界不清时，保留或询问人类。

任何可能改变外部行为、错误处理、数据口径、兼容逻辑、状态机、并发顺序、持久化格式、日志 / 指标语义或用户可见结果的处理，都只能作为候选报告并先询问人类。
...
- 默认 `$to-consolidation`：审计当前范围，并自动处理证据充分、未触及 P0 Boundary 的 C1 / C2 项。
- 用户要求 `audit-only`、只审计或只报告时：只输出 finding，不修改代码。
- 用户点名 `apply C-001 C-002` 时：只执行指定 finding；finding 在执行前仍必须满足当前删除证明和 P0 边界。

模式只控制是否修改，不放宽范围、取证或人类 gate。
...
2. **候选侦察**：用 `audit-method.md` 的 Candidate Recon 查找过度防御、已可替代的手写工具、旧测试 / 文档残留、半废弃 wiring 和重复 owner。候选只是调查入口，不构成删除证明。
3. **恢复目标意图**：从 source spec、用户确认、diff 和相邻代码提取新实现必须接管的职责。证据不足且会影响产品或数据语义时询问人类。
4. **建立 replacement map**：记录被替代的旧入口、字段、状态、缓存、调度、配置、测试口径和文档口径，以及对应新 owner。不要只依靠文本相似性推断替代关系。
5. **纵向追踪能力**：从 trigger / entry 追到 domain decision、scheduler、IO / persistence、state transition、consumer、side effects 和 observability。不能只按文件逐个扫。
6. **检查职责接管**：确认入口、数据模型、并发调度、副作用和证据口径没有新旧并存、重复 owner、重复控制或未消费结果。详细检查矩阵见 `audit-method.md`。
7. **建立删除证明**：删除或内联符号前，确认声明、生产者、消费者、持久化、外部边界、反射 / 注册、测试 / 运维引用和替代职责。证据不完整则不得自动处理。
8. **分类 finding**：分为可直接删除、可直接合并 / 内联、需要人类确认、明确保留。存在微小行为差异时，说明实际差异与简化理由；边界不清则进入人类确认。保留项必须说明其领域、契约、测试 seam 或观测价值。
9. **执行低风险收敛**：只处理证据充分、未触及 P0 Boundary 的目标范围问题。新增抽象排在最后；第一次文本重复通常保留清晰重复。
10. **定点复查与验证**：复查修改 hunk、真实调用链和工作区状态。运行与实际修改匹配的最小验证；无法验证时说明缺口。
...
- 使用语言标准库或已批准依赖替换当前目标内、行为等价的手写工具。
- 临时日志、probe、断点、一次性验证代码和注释掉的旧代码。
- 无歧义的旧注释、测试描述、局部命名和已失效配置；测试仅覆盖已删除行为且删除证明完整时，可同步删除对应测试、fixture 或 mock。
