...
`to-issue` 创建或更新 Sky Flow `issue` artifact。它把问题、证据、线索、未进入实施的改进候选，或从 spec / plan / 讨论中拆出的可独立认领工作单元，写入本地 `${SKY_FLOW_ROOT}/issue/`。它不创建 GitHub Issue，不调用外部 issue tracker。
...
5. 创建或更新 `${SKY_FLOW_ROOT}/issue/<issue-id>.md`；如果项目 docs 入口声明了其他 issue 目录或 TOC 规则，按本地规则执行。
6. 如果本地 docs 入口要求维护 TOC，创建、删除或移动 issue artifact 后同步更新 `${SKY_FLOW_ROOT}/AGENTS.md` / `${SKY_FLOW_ROOT}/CLAUDE.md` 中对应目录的 TOC。
7. 创建或修改 Sky Flow artifact 后运行 `validate-flow`，处理结构错误后再交付。
