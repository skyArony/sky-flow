...
9. 创建或修改 spec 后由本 Skill 的就绪证明和 Self-Review 收口语义一致性；只要求设计产物时请用户 review。用户明确要求大型分阶段交付时进入 `$to-milestone`，明确要求直接实现且没有人类决策未关闭时进入 `to-implement`。
...
- ready spec 的大型分阶段交付：显式 `$to-milestone`；单一清楚目标执行或继续：`to-implement`，由它决定 runtime-only 或按需 thin plan；从多个 spec 选择 / 恢复目标：`pick-goal`。
...
创建或更新 spec 后完成 Self-Review 并明确就绪状态。是否停下来等 review 取决于用户当前请求和仍存在的人类决策，不设置额外固定 gate。
