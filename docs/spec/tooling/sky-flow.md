...
| `to-archive`         | 自动     | plan 完成后压缩 task / fan-in 执行记录，或用户要求归档、压缩、清理 completed plan。                     | 默认 summary-only；删除 task 文件前遵守 runtime 审批规则。 |
...
- `to-archive` 只管完成态压缩：把 completed plan 的执行期 task / fan-in 记录收敛成长期事实、关键决策和证据入口。它不新增 archive artifact，也不替代 acceptance / backlog / handoff。
...
10. Archive completion：plan 完成后调用 `to-archive` 压缩 task / fan-in 执行记录，只把长期事实、关键决策和证据入口留在 completed plan；standalone task 完成后精简自身正文并移入 `tasks/standalone/done/`。
...
`to-archive` 用于 plan 完成后的归档压缩，并定义 standalone task 完成后的自压缩规则。它不新增 `archive` artifact 类型，而是把 completed plan 自身收敛成最终可读的历史记录：只保留长期事实、关键决策、验证证据入口和必要 follow-up。

task 文件的默认定位是执行期脚手架，而不是永久真相源。plan 未完成时，task 文件用于 DAG、并行、fan-in 和恢复；plan 完成且归档摘要已覆盖关键事实后，默认使用 `summary-only` 策略清理 task：
...
standalone task 不压缩回 plan。完成后应把执行流水压成 task 自身的 `Summary`、`Facts`、`Decision Log`、`Evidence` 和必要 follow-up，设置 `status: completed`，并移入 `${SKY_FLOW_ROOT}/tasks/standalone/done/`。

只有存在下列情况时才保留完整 task 文件：

- 审计、合规、客户复盘或 review 需要完整执行粒度。
- 未来仍需要 task 级恢复上下文，例如刚完成后 24 小时内要衔接后续 plan。
- task 中有尚未压缩到 completed plan / acceptance / backlog / handoff 的关键事实、证据、争议或失败尝试。
- abandoned / deferred / blocked task 仍需要被 backlog 或 handoff 指向。
- 人类明确要求保留完整任务记录。
...
- completed Plan 可以通过 `to-archive` 压缩为 summary-only：长期事实、关键决策和证据入口保留在 plan 正文，展开 task 文件默认不长期保留。
...
plan-scoped task 的生命周期默认止于 plan 完成后的 `to-archive`。未完成时，task 是执行、并行和恢复的主要结构；完成后，task 中值得长期保留的事实、决策和证据应被压缩回 completed plan。除非存在审计、恢复、争议或人类明确保留要求，完整 plan-scoped task 文件不需要长期保存。
...
- 如果 completed plan 已经 `to-archive` 为 summary-only，所属 `tasks/<plan-id>/` 目录可以删除，并同步把 plan frontmatter 的 `tasks` 清空。
