...
`to-plan` 需要维护 `goal` 字段。`goal` 是当前 plan 的完成契约，用于生成 Codex `/goal` 提示词。
...
- 吸收主代理管理、子代理 ROI、进度真相源、恢复入口和 `/goal` 续跑的有价值理念，但只保留对执行层有直接价值的约束。
...
5. 输出两个可复制代码块，供开发者在新会话中继续推进。

两个代码块必须分开输出：

```text
/goal <由 plan.frontmatter.goal 生成的目标契约>
```

```text
<具体上下文和执行细节：工作区、plan 路径、当前状态、下一步、限制、验证方式、何时停下问人>
```

分成两个代码块的原因：`/goal` 和具体细节一起复制时，Codex 有概率无法识别 `/goal`；分两次复制更稳。
