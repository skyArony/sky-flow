...
人类验收、完成态归档压缩、backlog 回收到通用技术知识沉淀的完整协作链路。
...
- 通用知识笔记默认不是 workflow artifact，不进入 `artifact_type` 枚举；`to-knowledge` 写入普通 Markdown
  knowledge note，除非本地知识库另有 schema。
...
- 自动场景必须触发：debug、infra 查询 / 操作、BDD 回归固化、testing、review、commit、consolidation、acceptance、通用技术知识沉淀、completed plan 归档压缩、validate-flow、Sky Flow plan / task execution 命中时，不等待用户再次点名。
...
- `to-knowledge` 只管通用知识沉淀：业务无关、项目无关、可跨项目复用的技术事实、踩坑、工具 / 库特性和选型对比。它不替代 spec / issue / plan / backlog / handoff。
- `to-archive` 只管完成态压缩：把 completed plan 的执行期 task / fan-in 记录收敛成长期事实、关键决策、踩坑和证据入口。它不新增 archive artifact，也不替代 acceptance / backlog / handoff。

这些能力可以串联，但不能互相代偿：artifact/status 问题回 `validate-flow`，测试策略和测试 ROI 回 `to-test`，代码风险回 `to-review`，diff 熵值回 `to-consolidation`，完成态历史压缩回 `to-archive`。
...
`to-spec` 用于先做人类对齐，再在确认后生成或更新 `spec`。

它的首要交付物是共享理解，不是文件。spec artifact 只是把已经对齐的稳定意图、事实、术语、范围、acceptance scenarios、
可测试且无歧义的要求、关键决策和 `to-plan` handoff 写入长期设计真相源。
...
`to-spec` 需要先判断当前是 `alignment-only` 还是 `artifact-write`：用户只是在探索、brainstorm、澄清需求或问“怎么看 / 如何优化”时，
先停留在对话对齐；用户指定现有 spec 时，先读取并理解，但只有确认后的设计变化才写回；用户明确要求创建 / 更新 spec 且输入已经足够稳定时，
才可以进入写入。未确认时不能用 artifact 代替对齐。

对齐门禁按顺序推进：Context Gate 校准仓库事实和冲突；Intent Gate 追清 problem、outcome、audience、success criteria 和非目标；
Option Gate 对真实分叉给出 `2-3` 个互斥方案、取舍和推荐项；Design Gate 展示推荐设计的 scope、关键行为、边界场景、
契约 / 数据影响和风险并获得用户接受；Artifact Gate 通过后才写入 spec，除非用户明确要求记录一个仍带 blocking questions 的 draft；
Review Gate 在写入后请用户 review，用户要求修改时更新 artifact 并重新自检 / validate。
...
简单事项可以一次展示设计摘要；中等事项分段展示 problem / outcome、scope、关键行为、acceptance scenarios、decisions 和 risks；
复杂事项每个主要 section 后确认是否符合预期，用户不同意时回到对应事实、术语、场景或方案分支。
...
写入或更新 spec 后，`to-spec` 必须请用户 review artifact；只有用户确认或明确要求继续时，才推荐进入 `to-plan`。
...
`to-issue` 写入 `${SKY_FLOW_ROOT}/issue/`。已修复且 `status: completed` 的 issue 移入
`${SKY_FLOW_ROOT}/issue/fixed/`；非 `completed` issue 不应放在 `issue/fixed/`。
如果 fixed issue 后续被同一 root cause 的新证据推翻，必须移回 `${SKY_FLOW_ROOT}/issue/`，
把 `status: completed` 改成 `in_progress` 或 `not_started`，并记录 `Reopen Evidence` /
`Reopen Reason`。新症状或新 root cause 不捞回旧 issue，应创建新 issue 并引用旧 fixed issue。

如果 `${SKY_FLOW_ROOT}/AGENTS.md` 或
...
`to-knowledge` 用于沉淀通用技术知识，而不是项目工作流状态。

它的典型输入来自调研、排障、实现、review 或技术选型过程中顺手发现的可复用事实，例如：

- 基础设施、网络、Kubernetes、操作系统、云服务或运维工具的通用行为和风险。
- 基础组件、库、框架、CLI、协议或标准的能力边界、版本差异和常见坑。
- 全栈类型安全、服务通信、存储 / 缓存 / 队列、可观测性等通用技术选型对比。
- 长篇官方文档、RFC、release notes 或设计文章中值得快速复用的精简摘要。

`to-knowledge` 默认写入 `${SKY_FLOW_ROOT}/knowledge/`，并使用普通 Markdown note。knowledge note
默认不写 `artifact_type` frontmatter，不进入 `validate-flow` 的 workflow artifact 枚举；只有本地知识库另有 schema
时才按本地 schema 执行。

触发原则是“自动但轻量”：

- 已经有权威来源或当前任务中已取得可靠证据，且短时间内能写清楚时，可以自动创建或更新知识笔记。
- 如果 runtime 支持子代理，知识沉淀可以作为旁路子代理任务执行；主代理继续推进主线，只 fan-in 最终笔记路径、关键结论和来源质量。
- 需要额外长时间调研、登录、生产系统证据或会明显拖慢主任务时，只推荐 `to-knowledge`，不强行切换。
- 一次发现多个候选时，优先沉淀最高复用价值的 `1-3` 条，避免把主任务变成知识库整理。

模型选择遵循足够原则：事实和来源已确认的短笔记使用低成本模型；有限范围的多方案对比使用中等模型；需要重新查证最新信息、处理来源冲突、压缩长文档或判断安全 / 基础设施风险时，才使用更强模型或回到主代理确认。

知识笔记必须干练：先写结论和适用场景，再列注意点和权威来源链接。不要搬运原始文档全文、教程大段内容或 API 列表。
长文档 summary 只保留会影响开发决策、运行风险、选型和排障路径的信息。

边界：

- 项目业务事实、当前 feature 设计、计划、任务、验收和 handoff 不进入 knowledge；分别交给对应 workflow artifact。
- 内部 URL、账号、密钥、客户信息、生产事故细节和项目专属部署拓扑不进入 knowledge。
- 未查证的论坛观点、模型记忆或传闻不能写成事实；只能作为待验证线索。
...
- Scope Check：如果输入只是单一可恢复工作单元、无需 milestone / task DAG / 长期验收 gate，可推荐 `to-task` 创建 standalone task；如果输入 spec / issue 覆盖多个独立子系统，`to-plan` 不硬塞进一个 standalone plan；应拆成多个 plan，或升级 parent / child plan。
...
- `to-plan` 可以要求 `design_review_gate` 作为 hard stop：plan 记录 gate 的完成条件和人类 approval 要求；task DAG 只承接 Agent 可执行的 consolidation / review / 证据整理，人类批准本身不建 task。
...
`to-task` 用于生成或更新 `task`。默认路径是从 `plan` 生成 plan-scoped task DAG；当工作比日常对话复杂、需要可恢复状态和验证意图、但仍是单一执行单元时，也可以从当前会话创建 standalone task。

plan-scoped task 必须以 `plan` 文档作为输入。它负责拆分 task、定义 task 类型、依赖、被依赖、并行关系和外部 plan 依赖。standalone task 不绑定 plan、不维护 peer task DAG，只记录一个可由 Agent 独立完成的工作单元。`to-task` 根据复杂度和阶段性特征决定是否在 task 内部拆 `step`。
...
- 只有 Agent 可以独立执行、判断完成并回传证据的工作才创建 task；真实设备、真实账号、外部环境、人工审批、人工体验判断或缺少权限的验证项不创建 task，改为创建或更新 acceptance。
- 代码改动和文档 / artifact 更新应作为可并行 lane 优先表达：实现 worker 写代码时，documentation worker 或主会话可以同步更新 spec、plan、task、acceptance、handoff 或交付文档；最终状态和完成结论由主会话 fan-in 对齐。
- 同一 artifact、同一 frontmatter 状态字段、同一 plan / task status 必须 single writer；非 owner 代理只回报事实、证据或 patch 建议。
...
`to-implement` 用于执行 Sky Flow `plan` / `task`，是 Sky Flow 的执行协调器。
...
- `to-implement` 负责执行层：读取 task DAG 或 standalone task、派发 / 执行 task、维护 runtime plan、验证、review、收敛、验收 gate、fan-in、状态回写和执行期 plan / task 调整。
...
10. Archive completion：plan 完成后调用 `to-archive` 按 summary-only 压缩 task / fan-in 执行记录，只把长期事实、关键决策、踩坑和证据入口留在 completed plan；standalone task 完成后按 completed plan 标准精简自身正文并移入 `tasks/standalone/done/`。
11. Dynamic maintenance：执行事实要求拆分 task、调整依赖 / 并行关系或补充验证 / 收敛 task 时，在已确认 scope 内按 `to-task` 规则更新 artifact 并运行 `validate-flow`。
12. Stop on blockers：遇到关键歧义、验证反复失败、scope / contract / 数据口径变化或高风险操作时，停止并询问人类。
...
- 代码改动和文档 / artifact 更新默认并行，而不是等代码全部完成后串行补文档。实现 worker 可以推进代码，documentation worker 或主会话同步维护 spec、plan、task、acceptance、handoff、README 或其他交付文档。
...
`to-archive` 用于 plan 完成后的归档压缩，并定义 standalone task 完成后的自压缩规则。它不新增 `archive` artifact 类型，而是把 completed plan 自身收敛成最终可读的历史记录：只保留长期事实、关键决策、验证证据入口、踩坑和必要 follow-up。

plan-scoped task 文件的默认定位是执行期脚手架，而不是永久真相源。plan 未完成时，task 文件用于 DAG、并行、fan-in 和恢复；plan 完成且归档摘要已覆盖关键事实后，固定使用 `summary-only` 策略清理 task：

- completed plan 保留 `Archive Summary`、`Facts`、`Decision Log`、`Evidence` 和必要 follow-up。
- plan frontmatter 的 `tasks` 清空为 `[]`。
- 删除 `${SKY_FLOW_ROOT}/tasks/<plan-id>/`；删除前必须确认当前 runtime 的 destructive command 审批规则。
- 若本地 docs TOC 或 artifact 引用包含 task 路径，必须同步更新。

standalone task 不压缩回 plan。完成后应按 completed plan 的归档标准压缩自身正文，保留短 `Summary`、`Facts`、`Decision Log`、`Evidence` 和必要 follow-up，设置 `status: completed`，并移入 `${SKY_FLOW_ROOT}/tasks/standalone/done/`。

plan-scoped task 不做归档捞回。后续发现问题时，追加新 task 或重新拆 task DAG；不要把旧 task 从 completed / 删除状态来回恢复成执行单元。

如果审计、合规、客户复盘、review、后续衔接、未完成项或人类明确要求仍需要 task 级事实，
不能把唯一事实来源直接删除；先把必要事实迁移到 completed plan / acceptance / backlog / handoff /
evidence 入口，或停下来让人类确认。标准归档结果仍是 `Task retention: summary-only`，不把完整
plan-scoped task 文件作为长期形态。

归档正文优先短而有用，重点保留决策日志、发现的坑、关键设计锚点、关键类 / 接口 / 状态 / 字段 / 关键词、验证结论和证据入口：
...
- 不复制完整 task 正文、长日志、聊天摘要、子代理原始汇报或命令流水。
- 不记录代码修改流水，不写长篇“改了哪个文件哪一行哪个函数”；只有关键类、关键设计、关键接口、关键状态或关键词本身对后续维护有索引价值时才保留。
- 不把未完成事项埋进归档；仍需恢复的内容必须转入 backlog / handoff。
- 不为了压缩而删除唯一证据；先迁移事实，再删除脚手架。
- 不改变 spec 需求、plan goal 或完成口径；发现完成口径不成立时回到 `to-implement` / `to-plan`。
- 压缩后必须运行 `validate-flow`，确认 completed plan、task 清理和 artifact 引用没有漂移。
...
它优先降低误报，不为了凑数量输出弱问题。用户可见输出默认使用中文标题和加粗中文字段 label，必要术语、枚举值、模型名和 finding id 保留英文；`Residual Risks` 等区块应写成 `残余风险`。每个 finding 最前面必须用显眼样式回答两个 ROI 问题：这个问题在实际场景下会出 Bug 吗？修它的代价是什么？
...
高风险或大范围变更可以使用 multi-review lane：多个 reviewer 独立审同一 scope，再 synthesize 成一份清晰清单。合成清单必须包含
`reviewer_agreement`、真实 bug 风险、修复成本、证据、影响、建议动作和 confidence。多 reviewer 共识提高确定性，单 reviewer
独有发现按触发路径、影响和修复成本排序，不自动丢弃。

当 `to-review` 用于验证已选 review findings 是否修复时，进入 verifier stage。至少两个 verifier 必须使用不同模型；
优先不同供应商，只有一个供应商时使用最新模型和次新模型。无法完成双 verifier 时，输出必须标注 `dual_verifier: unavailable`，不能宣称完成双 verifier 验收。
...
```text
目标: <由 plan.frontmatter.goal 或 standalone task.frontmatter.goal 生成的目标契约>
...
- `artifact_type`、`status`、`task_type`、`task_role`、`acceptance_type` 是否使用合法枚举。
...
- `status: completed` 的 fixed issue 必须移入 `${SKY_FLOW_ROOT}/issue/fixed/`；非 `completed`
  issue 不应放在 `issue/fixed/`。
- fixed issue 被同一 root cause 的新证据推翻时，必须移回 `${SKY_FLOW_ROOT}/issue/`，把
  `status: completed` 改成 `in_progress` 或 `not_started`，并在正文记录 `Reopen Evidence` /
  `Reopen Reason`。新症状或新 root cause 应创建新 issue，并引用原 fixed issue。
...
- `Fixed Summary`：issue 已修复并移入 `issue/fixed/` 后，用短结论保留 resolution、fixed by、
  verification、residual risk 和 follow-up。
- `Reopen Evidence` / `Reopen Reason`：fixed issue 被捞回时必须写，说明新证据和为什么原完成结论不再成立。
...
- completed Plan 可以通过 `to-archive` 压缩为 summary-only：长期事实、关键决策、踩坑和证据入口保留在 plan 正文，展开 task 文件默认不长期保留。
- completed Plan 被新证据推翻且仍属于同一 goal / scope 时，必须从 `plan/done/` 移回
  `${SKY_FLOW_ROOT}/plan/`，把 `status: completed` 改成 `in_progress` 或 `not_started`，并在正文记录
  `Reopen Evidence` / `Reopen Reason`。后续增强、二期或相邻问题不捞回旧 plan，应创建新 plan 并引用 completed plan。
...
- `plan` 通过 frontmatter 绑定当前保留的直属 plan-scoped `task` 列表；completed plan 已经 summary-only 压缩时，`tasks: []` 表示展开 task DAG 不再作为长期记录保留。
...
- `Archive Summary` / `Facts` / `Evidence`：只有 completed plan 经 `to-archive` 压缩后才需要；用于替代展开 task 细节。
- `Reopen Evidence` / `Reopen Reason`：completed plan 被捞回时必须写，说明新证据和为什么原完成结论不再成立。
...
- Archive check：completed plan 若已压缩，是否只保留事实、决策、证据和 follow-up，且未把唯一证据留在已删除 task 中。
- Boundary check：是否提前写入 implementation details、step-by-step code、命令清单或 commit 粒度；这些应留给 `to-task`。

### Task

`task` 是不大不小的具体任务。它有两种角色：

- `task_role: plan_scoped`：归属于某个 `plan`，是 plan task DAG 的节点。
- `task_role: standalone`：不归属于 plan，用于比日常对话复杂、需要可恢复状态和验证意图、但仍不值得创建 plan 的单一工作。

它通常由主会话拆出，并一般由子代理承接执行；也可以在某个新会话由主代理承接。主会话主要维护 plan runtime
进度、artifact 状态和 fan-in。standalone task 没有 parent plan，主会话只维护 task 自身状态和恢复入口。
...
- 与 `plan` 类似，使用编号前缀和名称。
- plan-scoped 格式：`01-xxx-xxx.md`；编号只在所属 `plan` 范围内保证排序。
- plan-scoped task 必须以所属 `plan` 名作为目录名存放，目录下再放具体 task 文档；示例：`tasks/001-example-plan/01-example-task.md`。
- plan-scoped task 完成时只更新 `status: completed`，仍保留在 `tasks/<plan-id>/`；不使用 `tasks/<plan-id>/done/`。
- standalone 格式：`t001-xxx-xxx.md`；编号在 standalone task 集合内保证稳定排序。
- standalone task 未完成时位于 `tasks/standalone/<task-id>.md`，完成后位于 `tasks/standalone/done/<task-id>.md`。
- 如果 completed plan 已经 `to-archive` 为 summary-only，所属 `tasks/<plan-id>/` 目录应删除，并同步把 plan frontmatter 的 `tasks` 清空。

定位规则：

- plan-scoped task 必须记录所属 `plan`。
- plan-scoped task 可以依赖同一 `plan` 下的其他 `task`。
- plan-scoped task 可以依赖外部 `plan` 下的 `task`，但必须写清外部 `plan` 和外部 `task` ID。
- plan-scoped task 需要显式记录：
  - 它依赖哪些 `task`。
  - 哪些 `task` 依赖它。
...
`acceptance` 承载真正需要人类参与的 gate：Agent 无法自行验证的真实环境行为、真实设备 / 账号 / 权限、人工体验判断、审批结论、阶段 sign-off、反馈和下一轮修订动作。它不是测试报告，也不是 task DAG 的替代品；Agent 可自证完成的检查留在 task / plan 证据里，Agent 无法完成的验收项进入 acceptance。
...
- 当 `to-task` 发现候选 task 无法由 Agent 独立完成，应创建或更新 plan 级 acceptance，而不是生成无法完成的 verification task。
