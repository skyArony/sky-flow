...
8. Artifact writeback：主代理更新 task status、plan progress / recovery / decision / blocker。
...
10. Archive completion：plan 完成后调用 `to-archive` 压缩 task / fan-in 执行记录，只把长期事实、关键决策和证据入口留在 completed plan。
...
`to-archive` 用于 plan 完成后的归档压缩。它不新增 `archive` artifact 类型，而是把 completed plan 自身收敛成最终可读的历史记录：只保留长期事实、关键决策、验证证据入口和必要 follow-up。
...
- `plan` 通过 frontmatter 绑定当前保留的直属 `task` 列表；completed plan 已经 summary-only 压缩时，`tasks: []` 表示展开 task DAG 不再作为长期记录保留。
...
task 的生命周期默认止于 plan 完成后的 `to-archive`。未完成时，task 是执行、并行和恢复的主要结构；完成后，task 中值得长期保留的事实、决策和证据应被压缩回 completed plan。除非存在审计、恢复、争议或人类明确保留要求，完整 task 文件不需要长期保存。
