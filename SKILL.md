---
name: sky-flow
description: 'Sky Flow workflow suite for artifact-based collaboration across spec, issue, plan, task execution, testing, acceptance, backlog, and handoff. Use when the user mentions Sky Flow, a to-* child workflow, pick-plan, validate-flow, workflow artifacts, skill migration, or asks to create, validate, progress, resume, execute a Sky Flow plan/task, test, accept, backlog, hand off, review, consolidate, or commit a file-backed workflow.'
---

# Sky Flow

Sky Flow 是通用工作流 Skill 套件，覆盖 `spec`、`issue`、`plan`、`task`、`acceptance`、`backlog`、`handoff` 等 artifact 的创建、推进、校验与衔接，用来把设计澄清、问题记录、实施编排、并行 fan-in、人类验收和跨会话交接沉淀成可恢复的工作流状态。
...
3. 选择子能力：
   - 用户显式点名子能力时，优先使用该能力。
   - 自动场景直接触发：debug、infra 查询 / 操作、BDD 回归固化、testing、review、commit、consolidation、acceptance、validate-flow、Sky Flow plan / task execution。
...
- `task`：`01-xxx-xxx.md`，必须位于 `tasks/<plan-id>/` 下，文件 stem 等于 `id`。
